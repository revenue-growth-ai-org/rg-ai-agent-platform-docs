...
This entry was platform-engineering drift in the platform's own base stack, not
something a customer hits while installing. It has moved to the
`1-rg-ai-agent-platform-base` repository (`TROUBLESHOOTING.md`), where the stack
it concerns actually lives.

It was also resolved on 2026-08-17 and this copy had not been updated, so it
described a decision as pending that had already been made.

The numbering is left in place so references to issues 1-14 stay stable.
