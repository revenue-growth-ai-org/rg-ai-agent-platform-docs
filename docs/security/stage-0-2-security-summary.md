...
| Production certificate protection | No explicit protection against accidental deletion of the production wildcard certificate by CI cleanup logic. | Explicit IAM Deny statement protecting ACM certificate `d830f3a3-a29b-40a7-aee7-db0fc10192ca` (`*.revenue-growth.ai`), ensuring no CI cleanup path can delete it. | IAM policy containing the explicit Deny on the certificate ARN. |
