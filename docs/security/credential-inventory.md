...
| `terraform-deploy` (IAM role) | AWS IAM role, assumable; no access keys | `AdministratorAccess` (AWS-managed), attached at creation | AWS account (IAM) — one per deployment account, including the CI account | Standing deployment-admin role. Its ARN is passed to every platform repo as `deployment_role_arn`; the base repo names it in the platform KMS key policy (`DeploymentRoleKeyAdministration`) | Created by `install.sh` and `master-setup.sh` when missing ([CUSTOMER-SETUP.md](../../CUSTOMER-SETUP.md) Step 4). Trust policy allows `sts:AssumeRole` from the account root, so any IAM principal in that account whose own policy permits assuming it can use it. **No Terraform provider assumes this role and no platform code uses its credentials** — Terraform runs with the operator's own credentials. Because it carries `AdministratorAccess`, it can read every secret and use the platform CMK (see [Secrets Access Map](./secrets-access-map.md#kms-decrypt-access-customer-account-cmk)). In the first production deployment (created 2026-07-27), IAM reports the role has never been assumed, so this is unused standing access. Not yet scoped or removed; CUSTOMER-SETUP.md describes replacing `AdministratorAccess` with a scoped policy. |
...
| Operator IAM user (customer deployment account) | AWS IAM user; CLI access through `aws login` | AdministratorAccess via an `Admin` group, plus `SignInLocalDevelopmentAccess` (the AWS-managed policy `aws login` requires) | AWS IAM in the customer's account; no access keys | Terraform applies, deploys and audits in the first production deployment. Terraform runs with this user's credentials — see `terraform-deploy` above | Verified live 2026-09-14: no access keys; console sign-in enabled; **no MFA device**; the only IAM user in the account. `aws login` issues credentials that expire after 15 minutes and are refreshed for up to 12 hours. Enabling MFA on this user is an open action. |
