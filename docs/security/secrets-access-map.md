...
|---|---|---|
| Deployment role (`terraform-deploy`) | **Every secret, and decrypt/encrypt with the platform CMK**, when it carries the default `AdministratorAccess` policy. The key policy gives it two routes to the key. Its own statement (`DeploymentRoleKeyAdministration`) grants administration actions rather than `kms:Encrypt` or `kms:Decrypt` — but those actions include `kms:PutKeyPolicy`, so the role can rewrite the key policy and grant itself use of the key. Separately, a statement granting the account root `kms:*` lets IAM policies authorise key use directly. Removing `AdministratorAccess` closes the second route, not the first. The IAM route was verified with the IAM policy simulator against a live deployment; the key-policy route is read from the base repo's key policy. | Customer-provisioned and customer-revocable. [CUSTOMER-SETUP.md](../../CUSTOMER-SETUP.md) attaches `AdministratorAccess` by default and documents replacing it with a scoped policy; doing so narrows the IAM route, but the key-policy route remains until the `DeploymentRoleKeyAdministration` statement is narrowed or the role is removed. No Terraform provider assumes this role — Terraform runs with the operator's own credentials — so this access is standing rather than exercised by deploys; see the [Credential Inventory](./credential-inventory.md). |
...
The deployment role has a further route through the key policy itself: its `DeploymentRoleKeyAdministration` statement includes `kms:PutKeyPolicy`, which lets it change who may use the key, including granting itself decrypt. That statement is in the base repo's key policy, so scoping the role's IAM policy does not remove it.
