...
The full policy (source of truth) is
[`scoped-deployment-policy.json`](./scoped-deployment-policy.json). **Do not
attach it directly as one document** — minified it's ~11.2 KB, which exceeds
both AWS's 10,240-byte limit for a role's combined inline policies and its
6,144-byte limit for a single customer-managed policy. Attach it as two
separate customer-managed policies instead:
[`scoped-deployment-policy-part-1.json`](./scoped-deployment-policy-part-1.json)
and
[`scoped-deployment-policy-part-2.json`](./scoped-deployment-policy-part-2.json).
These are generated by [`split-scoped-policy.py`](./split-scoped-policy.py)
— re-run that script and commit its output whenever
`scoped-deployment-policy.json` changes; do not hand-edit the `-part-N`
files.

## Before you apply it

Replace these placeholders throughout **both part files**:
...
Create each part as its own customer-managed policy and attach both to the
identity that runs `install.sh` / `master-setup.sh` / Terraform (a user or
role in your own account — this is **not** the same as the cross-account
role a remote MCP connector assumes; see `aws_trust_policy_example` in the
MCP tooling for that separate case):

```bash
for i in 1 2; do
  aws iam create-policy \
    --policy-name "${PROJECT_NAME}-scoped-deployment-part-${i}" \
    --policy-document "file:///path/to/scoped-deployment-policy-part-${i}.resolved.json"
  aws iam attach-role-policy \
    --role-name "$ROLE_NAME" \
    --policy-arn "arn:aws:iam::${AWS_ACCOUNT_ID}:policy/${PROJECT_NAME}-scoped-deployment-part-${i}"
done
```

(Resolve the `${...}` placeholders in each part file to a
`*.resolved.json` copy first, the same way as any other step here — `sed`
or `envsubst` both work.)
