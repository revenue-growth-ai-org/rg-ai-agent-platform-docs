...
| RDS final snapshot (only if a database was provisioned and later removed) | RDS, customer account | Until deleted | Taken automatically when an instance is destroyed. A later `destroy.sh` run keeps it — see Deletion below. |
