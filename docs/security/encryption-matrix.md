...
| RDS final snapshot — only if a database was provisioned and later removed | Snapshot of the removed instance | Yes | Same customer-managed CMK as the instance | Retained until deleted; `destroy.sh` keeps it unless run with `DESTROY_DELETE_KEPT_RDS_SNAPSHOTS=true` |
