...
| Secret scanning (gitleaks, CI) | **The four private repositories** | Gitleaks runs as a CI job on every push to `0-…-bootstrap`, `1-…-base`, `2-…-orchestrator` and `3-…-agent`. This — not GitHub-native scanning — is the control that covers the private repositories. The public docs repository does **not** run this job; it is covered by GitHub-native secret scanning and push protection instead (row above), so the two controls together cover all five. Note the CI job scans each pushed commit, not full history (the checkout is shallow); the separate one-time full-history sweep is documented in the [Stage 0–2 Summary](./stage-0-2-security-summary.md). |
